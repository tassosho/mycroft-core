...
class EnclosureMouth:
        ...
        if event and event.data:
            ...
            for code, end in visemes:
                ...
                if time.time() < start + end:
                    self.writer.write(f'mouth.viseme={code}')
    ...
    def text(self, event=None):
        ...
        self.writer.write(f"mouth.text={text}")
    ...
    def __display(self, code, clear_previous, x_offset, y_offset):
        ...
        clear_previous = f"cP={clear_previous},"
        x_offset = f"x={str(x_offset)},"
        y_offset = f"y={str(y_offset)},"

        message = f"mouth.icon={x_offset}{y_offset}{clear_previous}{code}"
        ...
        if len(message) > 60:
            ...
            message1 += "$"
            message2 += "$"
            message2 = f"mouth.icon={message2}"
    ...
    def display_weather(self, event=None):
        ...
        if icon is not None and temp is not None:
            icon = f"x=2,{icon}"
            msg = f"weather.display={str(temp)},{str(icon)}"
