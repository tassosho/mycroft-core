...
def start_qml_gui(messagebus, output_buf):
    ...
    bus.emit(Message("mycroft.gui.connected", {"gui_id": f"cli_{str(getpid())}"}))
...
def build_output_buffer():
    ...
    try:
        if skill:
            buffer.append(f"Active Skill: {skill}")
            buffer.append(f"Page: {basename(page)}")
            buffer.append("vars: ")
            for v in vars[skill]:
                buffer.append(f"     {v}: {vars[skill][v]}")
...
def handle_gui_ready(msg):
    ...
    if gui_id != f"cli_{str(getpid())}":
        # Not us, ignore!
        return

    if port := msg.data.get("port"):
        log_message(f"Connecting CLI GUI on {str(port)}")
        ws = websocket.WebSocketApp(
            f"ws://0.0.0.0:{str(port)}/gui",
            on_message=on_gui_message,
            on_error=on_gui_error,
            on_close=on_gui_close,
        )

        log_message(f"WS = {str(ws)}")
...
def gui_connect(ws):
    ...
    log_message(f"GUI Connected{str(ws)}")
...
def on_gui_message(ws, payload):
    ...
    try:
        ...
        log_message(repr(e))
        log_message(f"Invalid JSON: {str(payload)}")
...
def on_gui_error(ws, err):
    log_message(f"GUI error: {str(err)}")
