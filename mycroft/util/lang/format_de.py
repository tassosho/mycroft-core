...
def nice_number_de(number, speech, denominators):
    ...
    if whole == 0:
        return f'ein {den_str}' if num == 1 else f'{num} {den_str}'
    elif num == 1:
        return f'{whole} und ein {den_str}'
    else:
        return f'{whole} und {num} {den_str}'
...
def pronounce_number_de(num, places=2):
    ...
    def pronounce_fractional_de(num,
        ...
        while places > 0:  # doesn't work with 1.0001 and places = 2: int(
            ...
            result += f" {NUM_STRING_DE[int(num * place) % 10]}"
    ...
    def pronounce_whole_number_de(num, scale_level=0):
        ...
        if last_triplet == 1:
            if scale_level == 0:
                result += '' + 'eins' if result != '' else "eins"
            elif scale_level == 1:
                result += f'ein{EXTRA_SPACE}tausend{EXTRA_SPACE}'
            else:
                result += f"eine {NUM_POWERS_OF_TEN[scale_level]} "
        elif last_triplet > 1:
            result += pronounce_triplet_de(last_triplet)
            if scale_level == 1:
                # result += EXTRA_SPACE
                result += f'tausend{EXTRA_SPACE}'
            if scale_level >= 2:
                # if EXTRA_SPACE == '':
                #    result += " "
    ...
    if abs(num) >= 1000000000000000000000000:  # cannot do more than this
        ...
        return f"minus {pronounce_number_de(abs(num), places)}"
    else:
        if num == int(num):
            return pronounce_whole_number_de(num)
...
def pronounce_ordinal_de(num):
    ...
    if num < 0 or num != int(num):
        ...
        return f"{pronounce_number_de(num)}te"
    else:
        return f"{pronounce_number_de(num)}ste"
...
def nice_time_de(dt, speech=True, use_24hour=False, use_ampm=False):
    ...
    speak = ""
    if use_24hour:
        speak += "ein" if dt.hour == 1 else pronounce_number_de(dt.hour)
        speak += " Uhr"
        if dt.minute != 0:  # zero minutes are not pronounced, 13:00 is
            # "13 Uhr" not "13 hundred hours"
            speak += f" {pronounce_number_de(dt.minute)}"
        ...
        if dt.hour == 0:
            ...
            speak += 'ein' if dt.hour in [1, 13] else pronounce_number_de(dt.hour)
        else:
